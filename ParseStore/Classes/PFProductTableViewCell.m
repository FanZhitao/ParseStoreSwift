...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    if (self == [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        UILabel *labelQuantity = [[UILabel alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 50.0f, 20.0f)];
        labelQuantity.font = [UIFont fontWithName:@"HelveticaNeue-Medium" size:20.0f];
        labelQuantity.textColor = [UIColor colorWithRed:14.0f/255.0f green:190.0f/255.0f blue:255.0f/255.0f alpha:1.0f];
...
- (void)layoutSubviews {
    ...
    self.orderButton.frame = CGRectMake(x, y, 120.0f, 35.0f);
    
    self.labelQuantity.frame = CGRectMake(x + 130.0f, y + 4.0f, self.labelQuantity.frame.size.width, self.labelQuantity.frame.size.height);
}
...
- (void)configureProduct:(PFObject *)product {
    ...
    NSString *buttonTitle = [product[@"hasSize"] boolValue] ? NSLocalizedString(@"Select Qt. & Size", @"Select Quantity and Size") : NSLocalizedString(@"Select Quantity", @"Select Quantity");
        [self.sizeButton setTitle:buttonTitle forState:UIControlStateNormal];
