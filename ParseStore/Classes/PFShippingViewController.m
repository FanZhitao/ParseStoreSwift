...
- (instancetype)initWithCartItems:(NSArray *)items {
    if (self = [super initWithStyle:UITableViewStyleGrouped]) {
...
- (void)next:(id)sender {
    if ([self isValid]) {
        ...
        UIViewController *checkoutController = [[PFCheckoutViewController alloc] initWithItems:self.cartItems shippingInfo:shippingInfo];
...
- (void)addHeaderView {
    ...
    productImageView.file = self.cartItems[0][@"product"][@"image"];
    ...
    [self.cartItems enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        price += [obj[@"product"][@"price"] integerValue];
    }];
    priceLabel.text = [NSString stringWithFormat:@"$%ld", (long)price];
    ...
    nameLabel.text = self.cartItems[0][@"product"][@"description"];
...
- (void)addTextFields {
    ...
    self.cityStateField.autocapitalizationType = UITextAutocorrectionTypeNo;
    ...
    if (1) {
        // test data
        self.nameField.text = @"Zhitao";
        self.emailField.text = @"fan.zhitao.cn@gmail.com";
        self.addressField.text = @"87 Oneida Ave";
        self.cityStateField.text = @"Centereach, NY";
        self.postalCodeField.text = @"11720";
    }
}
