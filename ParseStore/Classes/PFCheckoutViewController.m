...
@property (nonatomic) NSArray<PFObject *> *items;

- (void)displayError:(NSError *)error;
- (void)charge:(STPToken *)token;
- (void)chargeItemsInCart:(STPToken *)token;
...
- (void)chargeItemsInCart:(STPToken *)token {
    ...
    [PFCloud callFunctionInBackground:@"purchaseItemsInCart"
                       withParameters:productInfo
                                block:^(id object, NSError *error) {
                                    [self.hud hide:YES];
                                    if (error) {
                                        ...
                                        PFFinishViewController *finishController = [[PFFinishViewController alloc] initWithProduct:nil];
                                        [self.navigationController pushViewController:finishController animated:YES];
                                        [[NSNotificationCenter defaultCenter] postNotificationName:@"CheckoutFinished" object:nil];
                                    }
                                }];
}
